...
function DrawingCanvas() {
  const { roomDetails, user, Socket, collabUsers, setRoomDetails } =
    useUserAndChats();
  ...
  const updateRoomDetails = useCallback(
    (userDetails) => {
      if (user._id !== userDetails.userId) {
        const updatedUsers = [...roomDetails.collaborators, userDetails];
        setRoomDetails({ ...roomDetails, collaborators: updatedUsers });
        var collabMap = new Map(
          JSON.parse(localStorage.getItem("collabUsers"))
        );
        collabMap.set(userDetails.userId, userDetails);
        localStorage.setItem("collabUsers", JSON.stringify(collabMap));
      } else {
        socket?.on("collab-room-join-details", (room) => {
          const updatedUsers = [...room.collaborators, userDetails];
          setRoomDetails({ ...room, collaborators: updatedUsers });
          const collabMap = new Map();
          room.collaborators.forEach((collaborator) => {
            collabMap.set(collaborator.userId, collaborator);
          });
          localStorage.setItem("collabUsers", JSON.stringify(collabMap));
          collabUsers.current = collabMap;
        });
      }
    },
    [collabUsers, roomDetails, setRoomDetails, socket, user?._id]
  );
  ...
  //   };
  // }, [socket, sharedMouseDown, sharedMouseMove, sharedMouseUp, setRoomDetails, updateRoomDetails, roomDetails]);
  ...
  const sharedMouseDown = useCallback(
    (data) => {
      ...
      collabUsers.current.set(userId, object);
      ...
      if (selectedTool === "PaintBucket") {
        ...
        var user = collabUsers.current.get(userId);
        user = { ...user, drawing: true };
        collabUsers.current.set(userId, user);
        context.beginPath();
        context.moveTo(x, y);
      }
    },
  ...
  const sharedMouseMove = useCallback(
    (data) => {
      ...
      const user = collabUsers?.current.get(userId);
